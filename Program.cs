...
namespace IT_Sec
{
    class Program
    {
        ...
        public static void Init()
        {
            ...
            if (input == "init")
            {

                {
                    ...
                    Rfc2898DeriveBytes rfcKey = new Rfc2898DeriveBytes(Console.ReadLine(), secretkey, 1000, HashAlgorithmName.SHA256);
                    byte[] vaultKey = rfcKey.GetBytes(16);

                    Dictionary<string, string> serverFile = new Dictionary<string, string>();
                    serverFile["IV"] = IV;          //måste omvandlas till string först
                    serverFile["Vault"] = encrypted; //måste omvandlas till string först
                    //myAes.Key = vaultKey;
                    // Encrypt the string to an array of bytes.
                    byte[] encrypted = EncryptStringToBytes_Aes(iv, vaultKey, myAes.IV);
        ...
        //Detta kan tas bort och så måste vi skapa en path för create Cfile / Sfile
